...
for (err_type in c("normal", "heavy")) {
  ...
  # p1_mv <- p2_mv <- p1_psis <- p2_psis <- numeric(n_sim) # 不再单独定义
  p_mv <- matrix(0, nrow = n_sim, ncol = R)
  p_psis <- matrix(0, nrow = n_sim, ncol = R)
  pall_mv <- pall_psis <- numeric(n_sim)
  
  for (sim in 1:n_sim) {
    dat <- generate_data(n = n, p = p, R = R, balanced = TRUE, heavy_tail = heavy_tail)
    X <- dat$X
    Y <- dat$Y
...
cat("\n========================================\n")
cat("实验三：十分类平衡场景 (n=200, p=2000, R=10)\n")
cat("========================================\n")
cat("模拟次数:", n_sim, "\n")
cat("筛选变量数 d =", d, "\n\n")
